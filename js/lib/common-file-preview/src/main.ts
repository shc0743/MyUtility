export type ConfigName = 
    'DisallowTextPreview' |
    'DisallowMediaPreview' |
    'DisallowAudioPreview' |
    'DisallowVideoPreview' |
    'DisallowImagePreview' |
    'DisallowOnlinePreview' |
    'DisallowPdfPreview' |
    'DisallowOnlineOfficeFilePreview' |
    'PreferredVolume';
...
const createDefaultConfigProvider = (): ConfigProvider => {
    const storage = new Map<ConfigName, string>();
    return {
        async get(name) {
...
export class HTMLCommonFilePreviewElement extends HTMLElement {
    ...
    constructor() {
        ...
        if (!this.shadowRoot) throw new Error('Failed to attach shadow root!')
    ...
    async init(
        get_url: (expiration?: number) => Promise<string>,
        fileType: string, fileName: string
    ) {
        if (this.#initbit) throw new Error('HTMLCommonFilePreviewElement has been initialized!');
        if (!this.#el) throw new Error('HTMLCommonFilePreviewElement is not created properly!');
        ...
        switch (majorType) {
            case 'audio':
            case 'video': if (false === (
                await configProvider.get('DisallowMediaPreview') == 'true' ||
                ...
                const apply_volume = (p: HTMLAudioElement | HTMLVideoElement) => {
                ...
                p.controls = true;
                if (p instanceof HTMLVideoElement) p.playsInline = true;
                apply_volume(p);
