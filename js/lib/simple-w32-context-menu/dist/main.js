...
export function AppendMenu(hMenu, type, config, data, event) {
    if (type === 'separator') {
    ...
    if (type === String) {
        ...
        if (config && config.submenu) {
            ...
            sub.addEventListener('pointerup', event);
        {
            let flags = false;
...
export function TrackPopupMenu(hMenu, x, y, flags = 0) {
    if (do_not_show_new_popup)
        return false;
    if (x === LAST_POSITION) {
    ...
    if (!hMenu.lastElementChild)
        throw new Error("Menu last element is null");
    const lastChild = hMenu.lastElementChild;
    if (x === 'center') {
    ...
    function evh(ev) {
        ...
        if (ev.target.classList.contains('disabled'))
            return false;
        if (ev.target === hMenu.lastElementChild)
            return false;
        close(hMenu);
        stop(ev);
    }
    hMenu.setAttribute('open', '');
    hMenu.querySelector('[tabindex]:not([disabled])')?.focus();
