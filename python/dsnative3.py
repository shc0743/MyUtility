#!/usr/bin/env python3
"""
DeepSeek thinking-mode + tool-call REPL PoC (requests) adapted for aliyun dashscope
Features:
...
# ========== Config ==========
BASE_URL = os.environ.get("DEEPSEEK_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1/")  # 可按需修改
API_KEY_PATH = Path(os.environ.get("API_KEY_PATH", "/data/data/com.termux/files/home/skapikeyali.txt"))
DEFAULT_MODEL = "deepseek-v3.2"  # 替换为你要用的模型标识
TIMEOUT = 60
...
TOOLS = [
    {
        ...
        "function": {
            ...
            "description": "Request to execute a shell command. The client must ask user confirmation before actual execution. Use 'cd' to change directory.",
            "parameters": {
                "type": "object",
                "properties": {
                    "command": {
...
def build_system_message(root_dir):
    return {
        "role": "system",
        "content": (
            "You are an assistant that can request execution of shell commands via a tool call. Only request the tool when you need to run commands.\n"
